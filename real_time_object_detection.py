...
vs = cv.VideoCapture('fluid_motion.MOV')
writer = None
time.sleep(2.0)
		...
		# greater than the minimum confidence
			...
			# draw the prediction on the frame
				     ...
				     COLORS[idx], 2)
                        ...
                        # if it is a bottle, get the area of bottle only
                        if label.partition(':')[0]=='person':
                                cropped_frame = frame[startY:endY,startX:endX]
                                cv.imshow('cropped', cropped_frame)
                                print("cropped size", cropped_frame.shape)
                                # Background subtractor
                                fgmask = fgbg.apply(cropped_frame)
                                grad, label, center = track_fluid(fgmask)
                                print("center",center)
                                cv.imshow('grad',grad)
        writer.write(frame)
